    ...
    <scroll-view class="calendar_body" :style="scrollHeight" :scroll-y="isScroll" :scroll-top="scrollTop" @scroll="scroll">
      ...
      <view class="calendar_meeting">
        ...
        <view v-for="(item,index) in meetingList" :key="index" :style="item.style" class="meeting_a" :class="item.bgClass"
          @click="showDetail(index,item,$event)">
        ...
        <!-- 创建会议 -->
        <view v-if='isCreate' class="meeting_create" :style="createMeet.style" @touchstart.stop="touchMeetingStart"
          @touchmove.stop="touchMeetingMove" @touchend.stop="touchMeetingEnd">
          <view class="radius_first radius" @touchstart.stop="moveTopStart(createMeet.idSt,$event)" @touchmove.stop="moveTopm"
            @touchend.stop="moveTopEnd">
            <view></view>
          </view>
          <view class="radius_second radius" @touchstart.stop="moveBottomStart(createMeet.idEnd,$event)"
            @touchmove.stop="moveBottomm" @touchend.stop="moveBottomEnd">
            <view></view>
          </view>
  ...
  export default {
      ...
      dayList(value){
        ...
        list.forEach(item=>{
          temp.push({
            ...
            startTime:item.scheduledStart,
            endTime:item.scheduledEnd,
            isMe:true,
            id:item.id
          })
        })
        console.log(temp);
        this.paramsList = temp;
        console.log(this.paramsList,'123123')
        this.getMeetingList();
      }
    },
    onLoad(){
      // this.$nextTick(()=>{
      //   console.log(this.dayList,'123123123')
      // })
    },
    created() {
    ...
    methods: {
      ...
      scroll(e) {
        ...
        scrollYtop = e.mp.detail.scrollTop;
      },
      //判断是否为今天
      isTodayFun(d) {
        let isToday;
      ...
      //整理会议列表数据
      getMeetingList() {
        console.log(this.dayList,'debugger')
        // let meetingList = [{
        //     isMe: true,
        ...
        //   }
        // ];
        let meetingList = this.paramsList;
        let list = [];
      ...
      showDetail(index, item,e) {
        ...
        let y = e.clientY;
        let newShow = this.meetingList[index],detailClass="",style="";
        if (y > 300) {
