			...
			<swiper-item>
				...
				<view class="date_dl" v-show="!retract || index == to_prev_week_index" v-for="(item,index) in   week_list_prev_co" :key="index">
					<view class="dd" @click="item_click(vo,index,key)" v-for="(vo,key) in item" :key="key">
						...
						<view v-show="vo.dot" class="dot"></view>
					</view>
				</view>
...
export default {
	...
	watch:{
		value(value){
			...
			this.get_date(Date.parse(value));
		},
	...
	methods:{
		...
		init(){
			...
			if(this.value){
				this.get_date(Date.parse(this.value));
			}else{
				this.get_date();
			}
		...
		doc_list_update(week_list = 'week_list'){
			...
			this[week_list].map((item,index)=>{
				list.push(item.map((vo,key)=>{
					if(this.dot_list.indexOf(vo.date) > -1){
						vo.dot = true;
		...
		item_click(item,item_index = -1){
			...
			this.nowTime = Date.parse(`${item.date}`);
			this.change()
			this.set_doc_lists_update();
		...
		get_month(type){
			...
			return Date.parse(`${nowYear}-${nowMonth}-${nowDay}`);
		},
