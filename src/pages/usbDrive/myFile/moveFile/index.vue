<template>
  <div class="wrap">
    <div class="container">
        <div class="content">
            <div class="row" v-for="(item,index) in folders" :key="index" @click="getCopyFiles(item,'folders')">
                <div class="lBox">
                    <p>
                        <img src="https://wx.phxinfo.com.cn/img/wechat/Folder.png" alt="">
                    </p>
                </div>
                <div class="rBox">
                    <h3 class="overflow">{{item.name}}</h3>
                    <p>{{item.createdOn}}</p>
                </div>
            </div>
            <div class="row" v-for="(item,index) in files" :key="index">
                <div class="lBox">
                    <p>
                        <img :src="item.fileExtension=='jpg'?item.link:'https://wx.phxinfo.com.cn/img/wechat/rar.png'" alt="">
                    </p>
                </div>
                <div class="rBox">
                    <h3 class="overflow">{{item.name}}</h3>
                    <p>{{item.createdOn}}</p>
                </div>
            </div>
        </div>
    </div>
    <div class="footer">
        <div class="box">
            <p @click="getAddNewFolders">新建文件夹</p>
            <p @click="getSubmit">移动到当前目录</p>
        </div>
    </div>
    <newFolder v-if="newShow" :ParentId="folderId" :newSign="newSign" @changeShow="changeShow" />
  </div>
</template>
<script>
import newFolder from '../../../../components/newFolder';
export default {
    components:{
        newFolder
    },
  data() {
    return {
      name: "",
      files: [],
      folders: [],
      imgList:[],
      copyFileId:"",
      folderId:"",
      strCode:"",
      newShow:false,
      newSign:0,
      sessionkey:""
    };
  },
  onLoad(options) {
    let sessionkey = wx.getStorageSync('sessionkey');
    this.sessionkey = sessionkey;
    this.name = options.fileName;
    this.folderId = options.fileId;
    console.log(this.folderId,'move');
    this.strCode = options.str;
    console.log(this.strCode);
    wx.setNavigationBarTitle({
      title: this.name
    });
    this.getQuery();
  },
  methods: {
    changeShow(isBook){
        this.newShow = false;
        this.getQuery();
    },
    getQuery() {
        this.$httpWX.get({
            url:this.$api.message.queryList,
            data:{
                method:"file.files.search",
                SessionKey:this.sessionkey,
                srchType:"my"
            }
        }).then(res=>{
            console.log(res);
            this.files = res.files;
            this.folders = res.folders;
            this.files.forEach(item=>{
                this.$set(item,'str',' files');
                if(item.fileExtension=='jpg'){
                    this.imgList.push(item.link)
                }
            })
            this.folders.forEach(item=>{
                this.$set(item,'str',' folders');
            })
        })
    },
    getCopyFiles(item){
        const url = '/pages/usbDrive/myFile/twoMoveFile/main?id='+item.id+'&name='+item.name+'&str='+this.strCode+'&moveId='+this.folderId;
        wx.navigateTo({url:url});
    },
    getAddNewFolders(){
        this.newShow = true;
    },
    getSubmit(){
        this.$httpWX.get({
            url:this.$api.message.queryList,
            data:{
                method:"file.directoryfile.move",
                SessionKey:this.sessionkey,
                Id:this.folderId,
                targetFolderId:"10010000-0000-0000-0000-000000000001",
                ObjectTypeCode:this.strCode=="folders"?"100200":"100100"
            }
        }).then(res=>{
            console.log(res,'move')
            let that = this;
            wx.showToast({
                title:res.msg,
                icon:'success',
                duration:2000,
                success:res=>{
                    that.getQuery();
                }
            })
        })
    }
  }
};
</script>
<style lang="scss">
@import "../../../../../static/css/public.scss";
.wrap {
    .container{
        padding-bottom: 80px;
    }
  .content {
    .row {
      display: flex;
      background: #fff;
      padding: 0 30rpx;
      align-items: center;
      .lBox {
        padding: 20rpx 0;
        p {
          width: 80rpx;
          height: 80rpx;
          img {
            width: 100%;
            height: 100%;
            vertical-align: middle;
          }
        }
      }
      .rBox {
        flex: 1;
        margin-left: 20rpx;
        border-bottom: 1rpx solid #f6f6f6;
        padding: 20rpx 0;
        h3 {
          font-size: 28rpx;
        }
        p {
          font-size: 12px;
          color: #999999;
        }
      }
    }
    .row:last-child .rBox {
      border: none;
    }
  }
  .footer{
        width: 100%;
        position: fixed;
        bottom: 20px;
        background: #fff;
        .box{
            display: flex;
            justify-content: space-between;
            padding: 31rpx;
            color: #3399ff;
            font-size: 29rpx;
        }
    }
}
</style>